...
// Named by the backend, not asserted here: DPK/Presidio only runs in the venv
// that has it installed, and both engines produce identical chunks.
const PII_ENGINE_LABEL: Record<string, string> = {
  "dpk-presidio": "Data Prep Kit",
...
const PRESETS = [
  {
    ...
    // Names the metric, not the fabrication. The figure the unprotected panel
    // invents is whatever the model invents that run - 12.5 on the current
    // build, 15 on an earlier one - and a hint that predicts a specific wrong
    // number is a claim the demo cannot keep.
    hint: "declared value is 10.4 days; the unprotected panel states a figure the log cannot produce",
  },
  {
              ...
              <div className="bg-[#11161d] border border-[#1d2530] p-3 rounded-md space-y-1.5 text-xs font-mono text-slate-400">
                {graph ? (
                  <>
                    ...
                    <Row k="PII masking" v={PII_ENGINE_LABEL[health?.pii_engine ?? ""] ?? "—"} ok={!!health} />
                  </>
                ) : (
